//
// Necessary .js libraries are included in-line.
//
...
(function () {
  ...
  $5WObject.prototype.title = function () {
    return 'Name' in this ? this.Name : this._id;
  }
  /*[
      </method>
    </class>
  ...
  $5WBrowserView = function($5w, el, type) {
    ...
    $(el).append('<div class="_5w_header"><div class="_5w_search_box"><input type="text" name="search"/><i class="fa fa-search"></i></div></div>');
    $(el).append('<div class="_5w_container"><ul class="_5w_search_list"></ul></div>');
    $('._5w_search_box input', el).on('input', function () { _this.refresh() });
  ...
  $5WBrowserView.prototype.refresh = function () {
    ...
    var ss = $('._5w_search_box input', this.el).val().toUpperCase();

    if (!ss) {
      $('._5w_search_list', this.el).empty();

      return;
    }

    this.$5w.fetchView('search', ss, null, ss + '\uffff')
      .done(function (data) {
        $('._5w_search_list', _this.el).empty();
        ...
        $.each(data.rows, function () {
          ...
          $('._5w_search_list', _this.el).append(li);
        });
      });
  }
  ...
  $5WBrowserView.prototype.listTap = function (e) {
    ...
    if (li.length) {
      $(li).trigger('5w_open_object', $(li).attr('data-id'));
    }
  }
  ...
  $5WViewerView = function($5w, el, type) {
    ...
    $5WView.call(this, $5w, el, type);
    $(el).append('<div class="_5w_header"><button class="fa fa-arrow-up">&nbsp;</button></div>');
    $('.fa-arrow-up', el).on('click', function () {
      $5w.popPane();
    });

    var c = $('<div class="_5w_container"/>');
    $(c).append('<div class="_5w_type_icon"></div>');
    $(c).append('<h1 class="_5w_obj_title">loading...</h1>');
    $(c).append('<h2 class="_5w_obj_owner"></h2>');

    $(el).append(c);

    var fh = new Hammer($(el)[0]);
  ...
  $5WViewerView.prototype.loadObject = function (id) {
    ...
    return this.$5w.fetchObject(id)
      .done(function (data) {
        var $5wo = new $5WObject(data);
        _this.$5wo = $5wo;

        $('._5w_obj_title', _this.el).text($5wo.title());

        for (key in $5wo) {
          if (data.hasOwnProperty(key)) {
            $('._5w_container', _this.el).append('<p data-fieldname="' + key + '"><label>' + key + '</label><span>' + $5wo[key] + '</span></p>');
          }
        }
      });
  }
  ...
  $5WViewerView.prototype.editField = function (fieldname) {
    ...
    var p = $('<div class="_5w_popup_form"></div>');
    var f = $('<form id="_5w_field_editor"><h1>' + fieldname + '<i class="_5w_close fa fa-times-circle"></i></h1></form>');
    $(f).append('<p><label><i class="_5w_scope"></i>Label</label><input type="text" name="label"/></p>');
    $(f).append('<p><label><i class="_5w_scope"></i>Hide</label><input type="checkbox" name="hidden"/></p>');
    $(f).append('<p><label><i class="_5w_scope"></i>Read Only</label><input type="checkbox" name="readonly"/></p>');
    $(f).append('<p><button type="submit">close</button></p>')
    $(p).append(f);

    this.$5w.overlayPane(p);
    ...
    $('#_5w_field_editor').submit(closeEdit);

    var ch = new Hammer($('#_5w_field_editor ._5w_close')[0]);
    ch.on('tap', closeEdit);

    // !!!TODO!!! figure out why tapping background doesn't dismiss modal form
    var bh = new Hammer($('._5w_popup_form')[0]);
    bh.on('tap', closeEdit);

    this.$5w.makeView($('._5w_scope'), 'scope');
  }
  ...
  $5WScopeView = function($5w, el, type) {
    var _this = this;

    $5WView.call(this, $5w, el, type);

    $(el).addClass('fa').addClass($5WScopeView.scopes[0]);
  ...
  $5WScopeView.prototype.cycleScope = function () {
    ...
    if (si > -1) {
      $(this.el).removeClass($5WScopeView.scopes[si]);
    }

    $(this.el).addClass($5WScopeView.scopes[++si%$5WScopeView.scopes.length]);
  }
  ...
  $5W.viewTypes = {'browser':$5WBrowserView, 'viewer':$5WViewerView,
      'scope':$5WScopeView};
  ...
  $5W.prototype.pushPane = function (pane) {
    $('._5w>div').slideUp();
    $('._5w').append(pane);
    $(pane).slideDown(function () { $(pane).trigger('5w_pane_visible'); });
  },
  ...
  $5W.prototype.popPane = function () {
    $('._5w>div:last').slideUp(function () { $(this).remove() });
    $('._5w>div:nth-last-child(2)').slideDown();
  }
  /*[
    </class>
  </package>
